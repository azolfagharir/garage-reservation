...
export default function MechanicDashboard() {
  ...
  const [modal, setModal] = useState<{ bookingId: number } | null>(null);
                ...
                {bookings.map((b) => (
                  <tr key={b.id} className="border-b border-gray-800 hover:bg-gray-800 transition">
                    ...
                    <td className="py-4 px-4 text-gray-400">
                      ...
                      {b.confirmedTime && <span className="mr-1 text-green-400">— {b.confirmedTime}</span>}
                    </td>
                    ...
                    <td className="py-4 px-4">
                      {b.status === "PENDING" && (
                        <div className="flex gap-2">
                          <button
                            onClick={() => { setModal({ bookingId: b.id }); setTimeInput(""); }}
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" dir="rtl">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-80">
            <h3 className="text-lg font-bold text-yellow-400 mb-4">تعیین ساعت نوبت</h3>
            <input
              type="time"
              value={timeInput}
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                onClick={() => {
                  if (!timeInput) return;
                  updateStatus(modal.bookingId, "CONFIRMED", timeInput);
                  setModal(null);
                }}
                className="px-4 py-2 rounded-lg bg-yellow-500 hover:bg-yellow-400 text-black font-bold text-sm"
              >تایید</button>
            </div>
          </div>
        </div>
